...
public class Bible {
	...
	private BufferedReader inKorean;
	private BufferedReader inEnglish;
	...
	public Bible() throws IOException {
		...
		InputStream kv = Bible.class.getResourceAsStream(koreanFileName);
		inKorean = new BufferedReader(new InputStreamReader(kv, "UTF8"));
    InputStream nkjv = Bible.class.getResourceAsStream(englishFileName);
	...
	public int findBook(String str){
		...
		for(int i = 0; i < LIST_ENG.length; i++){
			...
			if (bookName.contains(lower)|| LIST_KOR[i].equals(str)){
				return i;
			}
		}
